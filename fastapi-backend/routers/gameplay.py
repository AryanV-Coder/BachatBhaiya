...
def gameplay(body : GameplayRequest):
    ...
    system_prompt = '''
                        ...
                        6. The Winning Strategy Array: Determine the single best sequence of choices from Level 1 to Level 5 that maximizes the user's final coin balance. CRITICAL: You must output the human-readable sentences the user needs to click. DO NOT output the `next_node_id` (e.g., do not output "l2_canteen_lunch" or "success"). If the best choice is "Buy a second-hand book", you must output that exact string.

                        ### OUTPUT FORMAT
                        You must output ONLY a valid JSON array. Do not include markdown formatting like ```json or any conversational text.

                        The array must contain all the node objects first, followed by the winning strategy object as the very last element. Follow this exact schema:
                        [
                          {
